...
## Прокси для российских сетей (Yandex API Gateway)

Часть операторов (замечено на МТС LTE) душит соединения к Cloudflare,
за которым живёт Supabase: запросы замерзают без ошибки. Обход —
прокси на российской инфраструктуре, которую не трогают.

Тестовый вариант — API-шлюз Яндекс Облака (бесплатно до 100 000
запросов/мес, сервер и домен не нужны):

1. [console.yandex.cloud](https://console.yandex.cloud) → войти
   Яндекс-аккаунтом, создать платёжный аккаунт (пробный грант)
2. Сервис **API Gateway** → «Создать API-шлюз» → имя `pamyat-proxy`
3. В поле спецификации вставить целиком
   [`supabase/yandex-gateway.yaml`](../supabase/yandex-gateway.yaml)
4. Создать → скопировать **служебный домен**
   (`https://d5dxxx.apigw.yandexcloud.net`)
5. В `apps/client/.env` заменить адрес:
   ```
   EXPO_PUBLIC_SUPABASE_URL=https://d5dxxx.apigw.yandexcloud.net
   ```
...
~300 ₽/мес) — на него можно переехать той же одной строчкой в `.env`.
